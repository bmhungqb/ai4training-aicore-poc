...
def compute_decomposition(flows, masks, min_speed=0.3):
    """
    ...
    for i in range(N):
        ...
        if mask is not None:
            ...
            if len(hand_flow) > 0:
                # 1. Global Motion (Median)
                u_global = np.median(hand_flow[:, 0])
                v_global = np.median(hand_flow[:, 1])
                speed = np.sqrt(u_global**2 + v_global**2)
                
                # 2. Local Turbulence (Mean Absolute Deviation from Global Motion)
                diff_vectors = hand_flow - np.array([u_global, v_global])
                turbulence = np.mean(np.linalg.norm(diff_vectors, axis=1))

                global_speeds[i] = speed
                turbulences[i] = turbulence
                if speed >= min_speed:  # Only compute angle if moving reasonably
...
def smooth_linear(data, fps, window_sec=0.3):
    """Smooths linear data using Savitzky-Golay filter."""
    if len(data) == 0:
        return data
...
def smooth_circular(angles_deg, fps, window_sec=0.3):
    """Smooths circular data (degrees) using Gaussian filter on Sin/Cos components."""
    if len(angles_deg) == 0:
        return angles_deg
    
    # window_sec corresponds roughly to gaussian sigma
    sigma = max(1.0, (fps * window_sec) / 3.0) 
    
    # Handle NaNs (stationary hands)
    ...
    cos_vals = np.full_like(angles_deg, 0.0)
    sin_vals = np.full_like(angles_deg, 0.0)
    
    # Interpolate over NaNs for filtering to be smooth across gaps
    ...
    if len(valid_idx) > 0:
        ...
        cos_smooth = gaussian_filter1d(cos_interp, sigma=sigma)
        sin_smooth = gaussian_filter1d(sin_interp, sigma=sigma)
        
        # Only restore values where it was originally valid
        smoothed_rad = np.arctan2(sin_smooth, cos_smooth)
        smoothed[valid] = np.degrees(smoothed_rad)[valid]
...
def calculate_boundary_likelihood(speed_norm, turb_norm, shift_norm):
    ...
    # Speed valley means high likelihood
...
def run_multimodal_dynamic_segmentation(flows, left_masks, right_masks, fps, output_dir, min_speed=0.5):
    """
    ...
    print("  [Fusion] Computing metrics for LEFT hand...")
    l_spds, l_angs, l_turb = compute_decomposition(flows, left_masks, min_speed=min_speed)
    ...
    nl_shift = normalize_angle_shift(sl_angs, noise_threshold=30.0)
    l_likelihood, _ = calculate_boundary_likelihood(nl_spds, nl_turb, nl_shift)
    
    print("  [Fusion] Computing metrics for RIGHT hand...")
    r_spds, r_angs, r_turb = compute_decomposition(flows, right_masks, min_speed=min_speed)
    ...
    nr_shift = normalize_angle_shift(sr_angs, noise_threshold=30.0)
    r_likelihood, _ = calculate_boundary_likelihood(nr_spds, nr_turb, nr_shift)
    ...
    if window_size == 0:
        window_size = 1
    local_mean = ndimage.uniform_filter1d(overall_likelihood, size=window_size)
    mean_sq = ndimage.uniform_filter1d(overall_likelihood**2, size=window_size)
    local_std = np.sqrt(np.maximum(mean_sq - local_mean**2, 0))
    
    dynamic_threshold = local_mean + 1.0 * local_std
    dynamic_threshold = np.maximum(dynamic_threshold, 0.25)
    
    overall_peaks, _ = find_peaks(overall_likelihood, height=dynamic_threshold, distance=int(fps * 1.5))
